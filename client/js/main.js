      ...
      if (images.length > 0) {
        ...
        if (rd.privateChatTargetId) {
          ...
          if (targetClient && targetClient.shared) {
            ...
            addSystemMsg(
              `${t('system.private_message_failed', 'Cannot send private message to')} ${rd.privateChatTargetName}. ${t('system.user_not_connected', 'User might not be fully connected.')}`,
            )
          }
        } else {
          // 公共频道图片消息发送
      ...
      if (rd && rd.chat) {
        ...
        if (rd.privateChatTargetId) {
          ...
          if (targetClient && targetClient.shared) {
            ...
            addSystemMsg(
              `${t('system.private_file_failed', 'Cannot send private file to')} ${rd.privateChatTargetName}. ${t('system.user_not_connected', 'User might not be fully connected.')}`,
            )
          }
        } else {
          // 公共频道文件发送
